...
def view_menu(request, type_id, order_id):
    ...
    if request.session.session_key not in [emp_order.emp.session_id for emp_order in emp_orders if
                                           emp_order.emp.session_id != None]:
            ...
            try:
                ...
                if emp.is_signed_in:
                    ...
                    messages.error(request,
                                   'You have been BANNED PERMANENTLY! Please contact employee faculty if you think that there is something wrong')
                else:
...
def none_of_your_bussiness(request):
    ...
    if emp.n_o_w > 1:
        if emp.n_o_w < 3:
            messages.error(request,
                           'Caution ' + emp.username + ', this is the SECOND warning! You will receive PERMANENT BAN after THREE-TIME WARNING')
        elif emp.n_o_w == 3:
            messages.error(request,
                           'Caution ' + emp.username + ', this is the FINAL warning! You will receive PERMANENT BAN after THREE-TIME WARNING')
        else:
            messages.error(request,
                           'You have been BANNED PERMANENTLY! Please contact employee faculty if you think that there is something wrong')
            emp.sign_out()
        ...
        messages.error(request,
                       'Caution ' + emp.username + ', this is the FIRST warning! You will receive PERMANENT BAN after THREE-TIME WARNING')
...
def update_item(request, emp_id, order_id, item_id):
    ...
    if request.method == 'POST':
        ...
        if new_item.name == item.menu.name:
            ...
            order_item = OrderItem.objects.create(order=order, menu=new_item, quantity=new_quantity, note=new_note,
                                                  status=status)
            order_item.get_subtotal_cost()
            order_item.save()
            item.delete()
